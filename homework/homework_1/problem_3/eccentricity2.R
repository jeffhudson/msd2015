...
userlist$sat80 <- sapply(1:nrow(userlist), function(x) userlist$movies[[x]][ceiling(.8*userlist$n[x])])

userlist$sat70 <- sapply(1:nrow(userlist), function(x) userlist$movies[[x]][ceiling(.7*userlist$n[x])])
...
sat <- 
  data.frame(invsize = 1:length(inventory),
             sevnty = sapply(1:length(inventory), function(x) sum(userlist$sat70 <= x)),
             eighty = sapply(1:length(inventory), function(x) sum(userlist$sat80 <= x)),
...
satplot <- ggplot(data=sat, aes(x=invsize)) + 
  geom_line(aes(y=sevnty), color="darkorange") + 
  geom_line(aes(y=eighty), color="darkgreen") + 
